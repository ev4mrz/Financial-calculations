...
def simulate_and_price_bond(r0, gamma_star, r_star, sigma, T, delta, J,
                            maturities, maturity, coupon_rate, notional, frequency):
    """Run Vasicek Euler simulation and compute bond price statistics."""
    
    print(f"\nEULER SIMULATION: δ={delta}, J={J}")
    
    # Simulate short-rate paths
    rate_paths = simulate_short_rate_euler(r0, gamma_star, r_star, sigma, T, delta, J)
...
for J in [100, 1000, 10000]:
    simulate_and_price_bond(r0, gamma_star, r_star, sigma,
                            maturity, delta=delta, J=J,
                            maturities=maturities, maturity=maturity,
